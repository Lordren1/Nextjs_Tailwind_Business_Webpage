...
export default function Header() {
  ...
  return (
    <>
      <header 
        className={`fixed h-22 px-4 top-5 left-[2.5%] py-1 z-50 w-[95%] flex navbar items-center
          rounded-3xl transition-all duration-500
          ${sticky ? "bg-dark/800 backdrop-blur-lg shadow-lg" : "bg-dark"}`}
      >
        <div className="container mx-auto lg:max-w-(--breakpoint-xl) md:max-w-(--breakpoint-md) flex justify-between items-center xl:gap-16 lg:gap-8 py-6">
          ...
          <nav className="hidden lg:flex grow items-center justify-center space-x-10 text-base">
            {headerData.map((item, index) => {
              return (
                <HeaderLinks key={index} item={item}  sticky={sticky}/> 
              )
            })}
          </nav>
          ...
          <div className="nav-right gap-4 hidden lg:flex items-center">
             <Link
              href={"/Contact"}
              className="text-white bg-dark h-[50px] rounded-full font-chakrapetch
              font-semibold flex gap-2 ps-4 pe-2 py-2 w-auto justify-center items-center
              tracking-wider group"
            >
              Let's Talk
              <Icon
                icon="tabler:arrow-right" width="24" height="24"
            ...
            >
              <span className={`block w-6 h-0.5 
                ${ sticky ? "bg-dark hover:text-prim" : "bg-white hover:bg-prim-light"}
                `}></span>

              <span
                className={`block w-4 h-0.5  mt-1.5 transition-all duration-300 ease-in-out group-hover:w-6
                  ${ sticky ? "bg-dark hover:text-prim" : "bg-white hover:bg-prim-light"}
                  `}
              ></span>
      ...
      <div
        ref={mobileMenuRef}
        className={`lg:hidden fixed top-0 right-0 h-screen w-full sm:w-[50%] shadow-2xl bg-dark-blur
          backdrop-blur-lg p-10 z-50 transform transition-transform overflow-y-scroll duration-500 ease-in-out
      ...
      >
        ...
        <form className="search-box relative flex">
          ...
          <button
            type="submit"
            className="absolute right-0 h-full border-1 border-border cursor-pointer"
          >
